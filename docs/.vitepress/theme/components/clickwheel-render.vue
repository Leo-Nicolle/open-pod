...
const cutPath = computed(() => {
  const { W2, R, margin } = props
  const center = {
    x: R + margin,
    y: R + margin,
  };
  // radius of the circle
  const yTopRight = Math.sin(Math.acos(W2 / R)) * R;
  const xTopRight = Math.cos(Math.asin(W2 / R)) * R;
  const squarePoints = [
    // top right
  ...
  for (let i = 0; i < 4; i++) {
    ...
    path += `\nL ${s.x} ${s.y} L ${c1.x} ${c1.y} A ${R} ${R} 0 0 0 ${c2.x} ${c2.y}\n`;
  }
  return path;
});
const copperPath = computed(() => {
  let path = '';
  const { R, r, numPads, numCurves, gap, offset, roundness, margin } = props;
  for (let i = 0; i < numPads; i++) {
    ...
    let x1 = center.x + r * start.x;
    let y1 = center.y + r * start.y;
    for (let j = 0; j < numCurves; j++) {
      const radius = interp(r, R, (j + 1) / numCurves);
...
<template>
  <svg ref="clickwheel-svg" :width="`${width}mm`" :height="`${width}mm`" :viewBox="`0 0 ${width} ${width}`"
    ...
    <g v-if="showCopper" id="F.Cu" style="display:inline">
      <circle :cx="`${width / 2}`" :cy="`${width / 2}`" :r="`${R}`" fill="#cd4c33" mask="url(#coppermask)" />
    </g>
    <g>
      <slot></slot>
    </g>
