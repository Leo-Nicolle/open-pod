...
**CPU and memory**: single Arm Cortex-M33 core (with FPU, TrustZone, CORDIC/FMAC DSP
accelerators) at up to **160 MHz**, 4 Mbytes of flash, and **2512 Kbytes of SRAM** split
across five banks (SRAM1 768 KB, SRAM2 64 KB, SRAM3 832 KB, SRAM4 16 KB, SRAM5 832 KB —
SRAM4 and part of SRAM2 can be accessed by LPDMA1 in Stop modes, see §3), plus a separate
2 Kbyte backup SRAM and 128 backup registers in the always-on VBAT domain. Both flash and
RAM are an order of magnitude larger than typical for this pin count/price class, and the
2.5 MB of SRAM in particular is large enough to hold multi-second audio buffers or a
full FLAC/PCM decode pipeline entirely in RAM without touching flash or external PSRAM.
