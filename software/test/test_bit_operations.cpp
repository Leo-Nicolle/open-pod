...
TEST_CASE_FIXTURE(BitOperationsTestFixture, "2bpp transparency levels") {
    ...
    SUBCASE("All opaque (11)") {
        ...
        for (int i = 0; i < width * height; i++) {
            CHECK_EQ(COLOR_TEXT, displayBuffer[i]);
        }
    }
...
TEST_CASE_FIXTURE(BitOperationsTestFixture, "4bpp smooth gradients") {
    ...
    SUBCASE("Minimum transparency (0000)") {
        ...
        CHECK_EQ(COLOR_BACKGROUND, displayBuffer[0]);
    }
    ...
    SUBCASE("Maximum transparency (1111)") {
        ...
        CHECK_EQ(COLOR_TEXT, displayBuffer[0]);
    }
    ...
    SUBCASE("Intermediate transparency (1000)") {
        ...
        CHECK_NE(COLOR_BACKGROUND, blendedColor);
        CHECK_NE(COLOR_TEXT, blendedColor);
    }
    ...
    for (int w = 0; w < 5; w++) {
        for (int h = 0; h < 3; h++) {
            for (int b = 0; b < 3; b++) {
                ...
                REQUIRE_NOTHROW(renderer->renderTrackBinary("Test", buffer, width, height, 0, width));
...
TEST_CASE_FIXTURE(BitOperationsTestFixture, "Rendering quality comparison") {
    ...
    MESSAGE("Unique colors - 1bpp: ", uniqueColors1, ", 2bpp: ", uniqueColors2, ", 4bpp: ", uniqueColors4);
