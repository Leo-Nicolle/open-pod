...
bool MusicLookup::init() {
  ...
  uint32_t current_address = psram_base_address;

  // Load all indexes and relationships
  if (!loadIndexFromSDCard(artist_index_path, current_address, artist_index_h))
    return false;
  if (!loadIndexFromSDCard(album_index_path, current_address, album_index_h))
    return false;
  if (!loadIndexFromSDCard(genre_index_path, current_address, genre_index_h))
    return false;
  ...
  if (!loadRelationFromSDCard(artist_to_albums_path, current_address,
                              artist_to_albums_h))
    return false;
  Serial.println("Loading artist to tracks relation");
  if (!loadRelationFromSDCard(artist_to_tracks_path, current_address,
                              artist_to_tracks_h))
    return false;
  Serial.println("Loading album to tracks relation");
  if (!loadRelationFromSDCard(album_to_tracks_path, current_address,
                              album_to_tracks_h))
    return false;
  if (!loadRelationFromSDCard(genre_to_albums_path, current_address,
                              genre_to_albums_h))
    return false;
  if (!loadRelationFromSDCard(genre_to_tracks_path, current_address,
                              genre_to_tracks_h))
    return false;
...
bool MusicLookup::loadIndexFromSDCard(const char *filename,
  ...
  // Calculate layout:
  // [entry_count][data_size][ids...][offsets...][string_data...]
...
bool MusicLookup::loadRelationFromSDCard(const char *filename,
  ...
  uint32_t total_size =
      8 + source_ids_size + target_counts_size + target_ids_size; // 8 = header
  ...
  uint32_t id;
  uint32_t addr = header.baseOffset + 8;
  psram.readData(addr, (uint8_t *)&id, 4);
  Serial.printf("First id: %d", id);
  psram.readData(addr + 4, (uint8_t *)&id, 4);
  Serial.printf("Second id: %d", id);
...
lookup_result_t MusicLookup::getStringAtIndex(int32_t index,
  ...
  // Layout:
  // [header(8)][ids(entry_count*4)][offsets((entry_count+1)*4)][string_data]
...
uint32_t MusicLookup::getRelatedIds(uint32_t source_id,
  ...
  for (uint32_t i = 0; i < to_read; i++) {
    psram.readData(target_ids_base + ((target_offset + i) * 4),
                   (uint8_t *)&results[i], 4);
  }
...
int32_t MusicLookup::binarySearch(uint32_t id, uint32_t baseAddress,
                                  uint32_t count) {
...
bool MusicLookup::getAlbumName(uint32_t album_id, char *buffer,
                               uint32_t buffer_size) {
  return getString(album_id, album_index_h, buffer, buffer_size);
}

bool MusicLookup::getGenreName(uint32_t genre_id, char *buffer,
                               uint32_t buffer_size) {
  return getString(genre_id, genre_index_h, buffer, buffer_size);
}

bool MusicLookup::getTrackName(uint32_t track_id, char *buffer,
                               uint32_t buffer_size) {
...
void MusicLookup::printStats() {
  ...
  Serial.print("Artists: ");
  Serial.println(artist_index_h.entry_count);
  Serial.print("Albums: ");
  Serial.println(album_index_h.entry_count);
  Serial.print("Genres: ");
  Serial.println(genre_index_h.entry_count);
  Serial.print("Tracks: ");
  Serial.println(track_index_h.entry_count);
  Serial.print("Artist->Album relations: ");
  Serial.println(artist_to_albums_h.entry_count);
  Serial.print("Album->Track relations: ");
  Serial.println(album_to_tracks_h.entry_count);
  Serial.println("========================");
}