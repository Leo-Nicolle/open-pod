...
public:
  AnimationManager()
      : lastFrameTime(0), nextAnimId(1), isAnimating(false), frameCount(0),
        lastFPSUpdate(0), currentFPS(0), totalFrameTime(0),
        minFrameTime(UINT32_MAX), maxFrameTime(0) {
    animations.reserve(MAX_ANIMATIONS);
  }

  // Create a new animation
  uint32_t
  animate(AnimationType type, float startValue, float endValue,
          uint32_t duration, std::function<void(float)> updateFunc,
  ...
  uint32_t slideLeft(int distance, uint32_t duration,
                     std::function<void(float)> updateFunc,
                     std::function<void()> completeFunc = nullptr) {
    return animate(ANIM_SLIDE_LEFT, 0, distance, duration, updateFunc,
                   completeFunc);
  }
  ...
  uint32_t slideRight(int distance, uint32_t duration,
                      std::function<void(float)> updateFunc,
                      std::function<void()> completeFunc = nullptr) {
    return animate(ANIM_SLIDE_RIGHT, 0, distance, duration, updateFunc,
                   completeFunc);
  }

  uint32_t fadeIn(uint32_t duration, std::function<void(float)> updateFunc,
                  std::function<void()> completeFunc = nullptr) {
    return animate(ANIM_FADE_IN, 0, 1, duration, updateFunc, completeFunc,
                   Easing::easeOutQuad);
  }

  uint32_t fadeOut(uint32_t duration, std::function<void(float)> updateFunc,
                   std::function<void()> completeFunc = nullptr) {
    return animate(ANIM_FADE_OUT, 1, 0, duration, updateFunc, completeFunc,
                   Easing::easeInQuad);
  }
  ...
  bool update() {
    ...
    for (auto &anim : animations) {
      if (anim.isComplete || anim.isPaused)
        continue;
  ...
  void getPerformanceStats(uint32_t &avgFrameTime, uint32_t &minTime,
                           uint32_t &maxTime) {
