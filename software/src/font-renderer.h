...
public:
  ...
  void renderMenuItem(int number, const char* title, bool selected) {
    uint16_t bgColor = selected ? 0x001F : 0xFFFF;
    uint16_t textColor = selected ? 0xFFFF : 0x0000;
    uint16_t numColor = 0x4208;
    
    // Clear background (optimized)
    uint32_t fillColor32 = (uint32_t)bgColor | ((uint32_t)bgColor << 16);
    uint32_t* buf32 = (uint32_t*)buffer;
    for (int i = 0; i < (320 * 30) / 2; i++) {
      *buf32++ = fillColor32;
    }
    ...
    renderer.renderText(numStr, 5, 20, smallFont, numColor, bgColor);
    
    // Render title (with truncation if needed)
    int maxTitleWidth = 280; // Leave room for scroll bar
    int titleWidth = renderer.measureText(title, font);
    
    if (titleWidth <= maxTitleWidth) {
      renderer.renderText(title, 30, 15, font, textColor, bgColor);
    } else {
