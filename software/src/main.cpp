...
void setup() {
  ...
  string_results_t trackResults[MAX_SEARCH_RESULTS];
  uint32_t foundTracks = musicLookup.getTracksByArtist(
      djangoArtistId, trackResults, MAX_SEARCH_RESULTS);

  if (foundTracks == 0) {
    Serial.println("❌ No tracks found for Django Reinhardt!");
    while (1)
      delay(1000);
  }
